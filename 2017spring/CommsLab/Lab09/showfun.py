...
def showft(sig_xt, ff_lim, description="", disp="magphase"):
    """
    ...
    where  sig_xt: waveform from class sigWave
        ...
        ff_lim = [f1,f2,llim]
            f_low : lower frequency limit for display
            f_high: upper frequency limit for display
                (if f_low and f_high both =0, no f limit is placed on the FT display)
        llim = 0: display |X(f)| linear and absolute
        llim > 0: same as llim = 0 but phase is masked (set to zero) for |X(f)| < llim
        llim < 0: display 20*log_{10}(|X(f)|/max(|X(f)|)) in dB with lower display limit llim dB, phase is masked (set to zero) for f with magnitude (dB, normalized) < llim
        desctiption: Additional title information to be prepended to the generic title
        disp: Choose which plots to display ('magphase', 'mag', or 'phase')
    """
    ...
    if (disp=='magphase' or disp=='mag'):
        ...
        if disp=='mag':
            af11.set_xlabel('f [Hz]')
        af11.set_title(strgt)
        if (f_low==0 and f_high==0):
            xlim(xmin=0)
        else:
            xlim([f_low,f_high])
    if (disp=='magphase' or disp=='phase'):
...
def showpsd(sig_xt, ff_lim, N, subject='', zoom='',sqrd=''):
    ...
    where
        ...
        *** OUTPUTS ***
        Sxf: periodogram (PSD) of signal sig_xt
        ff: frequencies of the periodogram (x-axis)
    """
    ...
    where
        *** INPUTS ***
        sig_wt: waveform from class sigWave
            sig_wt.signal(): sampled CT signal x(t)
            sig_wt.get_Fs(): sampling rate of x(t)
        ixn: sample indexes for wn

        *** OUTPUTS ***
        (none)
    """
    wit = real(sig_wt.signal())
    wqt = imag(sig_wt.signal())

    wn = sig_wt.signal()[ixn]
    win = real(sig_wt.signal())
    wqn = imag(sig_wt.signal())

    maxwt=max(abs(concatenate((wit,wqt))))
    quick.quickplot(wit,wqt,'ob',win,wqn,'rx','Scatter Plot Of ASK Signal','$w_i(t)$','$w_q(t)$',[])#,[-maxwt,maxwt,-maxwt,maxwt],[10,10])
