...
def showft_plusplus(sig_xt, ff_lim, description=""):
    """
    Plot (DFT/FFT approximation to) Fourier transform of
    waveform x(t). Displays magnitude |X(f)| either linear
    and absolute or normalized (wrt to maximum value) in dB.
...
def showeye(sig_rt, FB, NTd=50, dispparms=[]):
    ...
    plotx=8
    ploty=4
    if (type(FB) is int or type(FB) is float):
        FB=[FB]
        plotx=15
        ploty=8
    nplots=len(FB)
    ...
    for fb in FB:
        ...
        title = 'Eye Diagram for sinc-Interpolated, Random Data (FB='+str(fb)+'bits/s)'
